...
var game = {
    ...
    start: function () {
        ...
        scoreAdd.timer.start();//分数增加
        this.isPause = false;
        cheatingDevice.speak.innerHTML = '游戏开始了哦！'
    },
    stop: function () {
        sky.timer.stop();
        land.timer.stop();
        ...
        scoreAdd.timer.stop();
        if(game.isOver){
            cheatingDevice.speak.innerHTML = '不要气馁哦！'
        }
    }
}
...
var hitManager = {
    ...
    validate: function () {
        if (bird.top >= land.top - bird.height) {
            //与大地亲吻
            cheatingDevice.speak.innerHTML = '请不要在地上摩擦！'
            return true;
        }
        //检查是否与柱子发生碰撞
...
hitManager.timer = getTimer(16, hitManager, function () {
    //检测是否碰撞
    if (cheatingDevice.isCheat) {
        if (this.validate()) {
            cheatingDevice.speak.innerHTML = '作弊器生效了，帮您取消了死亡判定！'
        }
    } else {
        if (this.validate()) {
            //碰撞了，游戏结束
            ...
            game.overDom.innerHTML = `游戏结束,作弊${cheatingDevice.cheatCount}次`
            game.isOver = true;
        }
    }
})
...
var cheatingDevice = {
    ...
    timer: null,
    cheatCount: 0,
    cheatBtn: document.getElementsByClassName('btn')[0],
    plan: document.getElementsByClassName('plan')[0],
    planImg: document.querySelector('.plan img'),
    tips: document.querySelector('.tips'),
    speak: document.querySelector('.speak'),
    planMove: function () {
        if (cheatingDevice.timer) {
            return;
        } else {
            var count = 0;
            cheatingDevice.planImg.style.top = '0px';
            cheatingDevice.timer = setInterval(function () {
                count++;
                if (count <= 10) {
                    console.log(count);
                    cheatingDevice.planImg.style.top = parseInt(cheatingDevice.planImg.style.top) + 37 + 'px';
                } else {
                    console.log(count);
                    clearInterval(cheatingDevice.timer);
                    cheatingDevice.plan.style.display = "none";
                    cheatingDevice.isCheat = false;
                    cheatingDevice.cheatCount++;
                    cheatingDevice.planImg.style.top = "0px";
                    cheatingDevice.timer = null;
                }
            }, 500)
        }
    }
}
cheatingDevice.cheatBtn.onclick = function () {
    if (game.isOver || game.isPause) {
        cheatingDevice.speak.innerHTML = `执行了无效的作弊操作，因为游戏正在暂停或者游戏已经结束!`
    } else {
        cheatingDevice.plan.style.display = "inline-block";
        if (cheatingDevice.cheatCount < 2) {
            cheatingDevice.tips.innerHTML = `<br>正在规避死亡伤害<br>第${cheatingDevice.cheatCount + 1}次`
        } else {
            cheatingDevice.tips.innerHTML = `<br>第${cheatingDevice.cheatCount + 1}次<br>多次作弊行为不可取`
            cheatingDevice.speak.innerHTML = '作弊一时爽，不如提高技术来得实在！'
        }
        cheatingDevice.isCheat = true;
        cheatingDevice.planMove();
    }
}
//分数增加
var scoreAdd = {
    sc: 0,//分数
    pn: 0,//pipeNumber 经过的管子数目
    dom: document.getElementsByClassName('score')[0],
}
scoreAdd.timer = getTimer(16, scoreAdd, function () {
    // console.log('计时器正常')

    if (!game.isPause || !game.isOver) {
        scoreAdd.sc++;
        if (scoreAdd.pn == 0) {
            scoreAdd.dom.innerHTML = `您的得分：${scoreAdd.sc}<br>您经过的柱子：0`;
        } else {
            scoreAdd.dom.innerHTML = `您的得分：${scoreAdd.sc}<br>您经过的柱子：${scoreAdd.pn + 1}`;
        }
        if (scoreAdd.sc > 2000) {
            cheatingDevice.speak.innerHTML = '你很棒！就这样，继续！'
        }
        if ( scoreAdd.pn > 25) {
            cheatingDevice.speak.innerHTML = '我要是能像你这么棒就好了呢'
        }
        if (scoreAdd.pn > 35){
            cheatingDevice.speak.innerHTML = '哇！！！你真是太棒了'
        }
    }
    else {
        clearInterval(scoreAdd.timer);
    }
...
window.onkeydown = function (e) {
    if (e.key === "Enter" || e.key === "f") {
        cheatingDevice.speak.innerHTML = '加油！你是最棒的鸟！'
        if (game.isOver) {
